    ...
    private void lookAhead(final LGraph layeredGraph,final int offset) {
        ...
        for (int i: modelOrderMask) {
            for (LNode source : layeredGraph.getLayerlessNodes()) {
                int groupID = source.getProperty(LayeredOptions.CONSIDER_MODEL_ORDER_GROUP_MODEL_ORDER_CYCLE_BREAKING_ID);
                if (groupID != i) {    
                    continue;
                }
                ...
                for (LPort port : source.getPorts(PortType.OUTPUT)) {
                    for (LEdge edge : port.getOutgoingEdges()) {
                        ...
                        int groupIDTarget = target.getProperty(LayeredOptions.CONSIDER_MODEL_ORDER_GROUP_MODEL_ORDER_CYCLE_BREAKING_ID);
    ...
    private HashSet<LEdge> getNextNodeWithGroupPriority(final LNode currentSource, 
        ...
        for (LPort port : currentSource.getPorts(PortType.OUTPUT)) {
            for (LEdge edge : port.getOutgoingEdges()) {
                ...
                LNode target = edge.getTarget().getNode();
                int groupPriorityTarget = target.getProperty(LayeredOptions.CONSIDER_MODEL_ORDER_GROUP_MODEL_ORDER_CYCLE_BREAKING_ID);
